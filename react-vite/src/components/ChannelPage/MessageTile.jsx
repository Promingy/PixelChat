    ...
    return (
      <>
              ...
              onEmojiClick={(e) => {
                ...
                for (let reaction of Object.values(message.reactions)) {
                  if (
                    ...
                    reaction.emoji == e.emoji
                  ) {
                    return dispatch(
                      removeReaction(channelId, message.id, reaction.id)
                    ).then(() => {
                      const payload = {
                        type: "reaction",
                        method: "DELETE",
                ...
                return dispatch(
                  initializeReaction(channelId, message.id, { emoji: e.emoji })
                ).then((res) => {
                  const payload = {
                    type: "reaction",
                    method: "POST",
        ...
        <div
          className={
            message.pinned
              ? "user-message-container pinned "
              : `user-message-container`
          }
          onMouseOver={() => setReactBar(true)}
          onMouseLeave={() => {
            setReactBar(false);
        ...
        >
          <div className="message-body-reactions-container">
            ...
            <div className="message-body-header-container">
              ...
              <div className="message-owner-date-container">
                ...
                >
                  <p className="message-owner" id={message.id}>
                    {user?.username}
                  </p>
                  <p className="message-post-time" id={message.id}>
                    {hours}:{minutes}
                  </p>
                  <p className="message-post-time" id={message.id}>
                    {amPm}
                ...
                <p className="message-body">
                  {parse(replaceClass(message.body))}
                  {/* {message.body} */}
                </p>
              </div>
            </div>
          ...
          {
            <div
              className={`react-bar${reactBar ? "" : " react-bar-hidden"} ${
                localStorage.getItem("theme") === "dark" ? "react-bar-dark" : ""
              }`}
            >
              <i
                className="fa-solid fa-face-laugh-wink fa-lg reaction-icon"
              ...
              {sessionUser.id === message.user_id && (
                <div>
                  <i
                    className="fa-regular fa-trash-can remove-message"
                    onClick={() => setConfirmMsgDel(!confirmMsgDel)}
                  />
                  {confirmMsgDel && (
                    <div
