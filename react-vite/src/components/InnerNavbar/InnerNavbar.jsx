        ...
        <div className={`inner-navbar-wrapper${showNavBar ? " do-show" : ""}`}>
          ...
          <ul className="inner-navbar-content">
            ...
            {Object.values(server.channels).map((channel) => (
              <li
                ...
                className={`${
                  type === "channel" && channel.id == channelId
                    ? " selected-channel"
          ...
          <ul className="inner-navbar-content">
            ...
            {Object.values(server.direct_rooms).map((direct_room) => (
              <li
                id={`channel${direct_room.id}`}
                key={direct_room.id}
                onClick={() => handleChannelUnbold(direct_room.id)}
                className={`${
                  type === "message" &&
                  (direct_room.owner_1_id == channelId ||
                    direct_room.owner_2_id == channelId)
                    ? " selected-channel"
                    : "not-selected-channel"
                }${direct_room?.bold ? " bold-channel" : ""}`}
              >
                <Link
                  to={`/main/servers/${server.id}/direct-messages/${
                    sessionUser.id === direct_room.owner_1_id
                      ? direct_room.owner_2_id
                      : direct_room.owner_1_id
                  }`}
                  className="inner-navbar-link"
                >
                  <div className="navbar-content">
